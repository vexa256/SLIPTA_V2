    ...
    <div x-data="auditResponseApp()" x-init="init()" class="space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
            ...
            <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm p-6 relative overflow-hidden">
                ...
                <div class="relative">
                    ...
                    <p class="text-xs text-neutral-500 mt-1"
                        x-text="scores.total_earned + ' / ' + scores.adjusted_denominator + ' pts'"></p>
            ...
            <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm p-6">
                ...
                <p class="text-xs text-neutral-500 mt-1"
                    x-text="progress.answered_non_na + ' / ' + progress.answerable_questions"></p>
            ...
            <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm p-6">
                ...
                <div class="mt-3">
                    <label class="inline-flex items-center cursor-pointer">
                        ...
                        <div
                            class="relative w-9 h-5 bg-neutral-200 rounded-full peer peer-checked:bg-indigo-600 transition-colors">
                            <div
                                class="absolute top-0.5 left-0.5 bg-white w-4 h-4 rounded-full transition-transform peer-checked:translate-x-4 shadow-sm">
                            </div>
                        </div>
        ...
        <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm p-4">
            ...
            <div class="relative">
                ...
                <div x-ref="sectionsContainer" @scroll="updateScrollButtons"
        ...
        <div class="bg-white border border-neutral-200 rounded-2xl shadow-sm p-6">
            <div class="flex items-start justify-between">
                <div>
                    ...
                    <p class="text-xs text-neutral-400 mt-1">Opened:
                        {{ \Carbon\Carbon::parse($audit->opened_on)->format('M d, Y') }}</p>
                </div>
                <div class="flex items-center gap-2">
                    @if ($readOnly)
                        <span
                            class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-medium bg-neutral-100 text-neutral-700 border border-neutral-200">
                            <i class="fas fa-lock text-neutral-400 mr-2"></i> Read Only
                        </span>
                    @endif
                    <div x-show="isSaving"
                        class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">
        ...
        {{-- Confirmation Modal --}}
        <div x-show="confirmDialog.show" x-cloak @keydown.escape.window="confirmDialog.show = false"
        ...
        <div x-show="evidenceModal.show" x-cloak @keydown.escape.window="evidenceModal.show = false"
            class="fixed inset-0 z-50 overflow-hidden" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
            x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100"
            x-transition:leave-end="opacity-0">
            ...
            <div class="absolute inset-4 md:inset-8 flex flex-col">
                ...
                <div class="flex-1 bg-neutral-800 rounded-2xl overflow-hidden flex items-center justify-center">
                    <template x-if="evidenceModal.type === 'image'">
                        <img :src="evidenceModal.url" :alt="evidenceModal.fileName"
                            class="max-w-full max-h-full object-contain">
                    </template>
                    <template x-if="evidenceModal.type === 'pdf'">
                        <iframe :src="evidenceModal.url" class="w-full h-full border-0"></iframe>
                    </template>
                    <template x-if="evidenceModal.type === 'other'">
                        <div class="text-center p-8">
                            ...
                            <a :href="evidenceModal.url" download
        ...
        <div class="space-y-6">
            <template x-for="(sectionData, sIndex) in sectionsData" :key="sIndex">
                <div x-show="!showOnlyUnanswered || sectionData.hasUnanswered" :id="`section-${sIndex}`"
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-neutral-600 mt-1"
                                    x-text="`${sectionData.answeredCount} of ${sectionData.totalCount} answered`"></p>
                            </div>
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-sm font-bold px-3 py-1 rounded-lg"
                                    :class="{
                                        ...
                                        'bg-amber-100 text-amber-700': sectionData.percentage >= 50 && sectionData
                                            .percentage < 75,
                            ...
                            <div x-show="!showOnlyUnanswered || !qData.isAnswered"
                                ...
                                <div class="flex items-start gap-4">
                                    <div class="flex-shrink-0 w-16 h-16 rounded-xl flex items-center justify-center border-2 transition-all"
                                        :class="{
                                            'bg-green-100 border-green-500': qData.response && qData.response
                                                .answer === 'Y',
                                            'bg-amber-100 border-amber-500': qData.response && qData.response
                                                .answer === 'P',
                                            'bg-red-100 border-red-500': qData.response && qData.response
                                                .answer === 'N',
                                            'bg-blue-100 border-blue-500': qData.response && qData.response
                                                .answer === 'NA',
                                            'bg-neutral-100 border-neutral-300': !qData.response
                                        }">
                                        <span class="text-sm font-bold"
                                            :class="{
                                                'text-green-700': qData.response && qData.response
                                                    .answer === 'Y',
                                                'text-amber-700': qData.response && qData.response
                                                    .answer === 'P',
                                                'text-red-700': qData.response && qData.response.answer === 'N',
                                                'text-blue-700': qData.response && qData.response
                                                    .answer === 'NA',
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-start justify-between gap-4 mb-3">
                                            <div class="flex-1">
                                                <!-- NEW: explicit question title line -->
                                                <h3 class="text-base font-semibold text-neutral-900 leading-tight">
                                                    <span x-text="qData.question.question_title || '—'"></span>
                                                </h3>

                                                <!-- Existing: question text -->
                                                <p class="mt-1 text-sm text-neutral-800 leading-relaxed"
                                                    x-text="qData.question.text"></p>

                                                <!-- Existing: ISO reference -->
                                            ...
                                            <div class="flex-shrink-0">
                                                <span
                                                    class="inline-flex items-center px-3 py-1.5 rounded-lg text-xs font-bold bg-indigo-600 text-white shadow-sm">
                                            ...
                                            <div class="mb-4 p-4 bg-neutral-50 rounded-xl border-2 border-neutral-200">
                                                <p class="text-xs font-semibold text-neutral-700 mb-3 flex items-center">
                                                    ...
                                                    <template x-if="qData.question.requires_all_subs_for_yes === 1">
                                                        <span
                                                            class="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 rounded-md text-xs font-medium">
                                                            <i class="fas fa-exclamation-triangle text-xs mr-1"></i>
                                                            All must be Y/NA for parent Y
                                                        </span>
                                                    </template>
                                                </p>
                                                <template x-for="(sub, subIdx) in qData.subquestions"
                                                    :key="subIdx">
                                                    <div
                                                        class="flex items-start gap-3 py-2.5 border-b border-neutral-100 last:border-0">
                                                        <span
                                                            class="text-xs font-semibold text-indigo-600 flex-shrink-0 mt-0.5 bg-indigo-50 px-2 py-1 rounded"
                                                            x-text="sub.sub_code"></span>
                                                        <p class="text-xs text-neutral-700 flex-1 leading-relaxed"
                                                            x-text="sub.text"></p>

                                                        @if (!$readOnly)
                                                            <div class="flex-shrink-0 flex gap-1.5">
                                                                <template x-for="opt in ['Y', 'P', 'N', 'NA']"
                                                                    :key="opt">
                                                                    <button
                                                                        ...
                                                                        :class="{
                                                                            'bg-green-500 text-white border-green-600': qData
                                                                                .sub_responses && qData.sub_responses[
                                                                                    sub.id] && qData.sub_responses[sub
                                                                                    .id].answer === opt &&
                                                                                opt === 'Y',
                                                                            'bg-amber-500 text-white border-amber-600': qData
                                                                                .sub_responses && qData.sub_responses[
                                                                                    sub.id] && qData.sub_responses[sub
                                                                                    .id].answer === opt &&
                                                                                opt === 'P',
                                                                            'bg-red-500 text-white border-red-600': qData
                                                                                .sub_responses && qData.sub_responses[
                                                                                    sub.id] && qData.sub_responses[sub
                                                                                    .id].answer === opt &&
                                                                                opt === 'N',
                                                                            'bg-blue-500 text-white border-blue-600': qData
                                                                                .sub_responses && qData.sub_responses[
                                                                                    sub.id] && qData.sub_responses[sub
                                                                                    .id].answer === opt &&
                                                                                opt === 'NA',
                                                                            'bg-white border-neutral-300 text-neutral-600 hover:bg-neutral-50':
                                                                                !qData.sub_responses || !qData
                                                                                .sub_responses[sub.id] || qData
                                                                                .sub_responses[sub.id].answer !== opt
                                                                        }"
                                                                        x-text="opt">
                                                                    </button>
                                                                </template>
                                                        ...
                                                        @else
                                                            <span
                                                                class="flex-shrink-0 inline-flex items-center px-2.5 py-1 rounded-md text-xs font-bold"
                                                                x-show="qData.sub_responses && qData.sub_responses[sub.id]"
                                                                :class="{
                                                                    'bg-green-100 text-green-800': qData
                                                                        .sub_responses && qData.sub_responses[sub.id] &&
                                                                        qData.sub_responses[sub.id]
                                                                        .answer === 'Y',
                                                                    'bg-amber-100 text-amber-800': qData
                                                                        .sub_responses && qData.sub_responses[sub.id] &&
                                                                        qData.sub_responses[sub.id]
                                                                        .answer === 'P',
                                                                    'bg-red-100 text-red-800': qData.sub_responses &&
                                                                        qData.sub_responses[sub.id] && qData
                                                                        .sub_responses[sub.id].answer === 'N',
                                                                    'bg-blue-100 text-blue-800': qData.sub_responses &&
                                                                        qData.sub_responses[sub.id] && qData
                                                                        .sub_responses[sub.id].answer === 'NA'
                                                                }"
                                        ...
                                        @if (!$readOnly)
                                            <div class="space-y-3">
                                                <div class="flex items-center gap-2 flex-wrap">
                                                    <template x-for="option in ['Y', 'P', 'N', 'NA']"
                                                        :key="option">
                                                        <button @click="selectAnswer(sIndex, qIndex, option)"
                                                            :disabled="(qData.subquestions && qData.subquestions.length > 0) || (
                                                                option === 'Y' && !canSelectYes(qData))"
                                                            :title="qData.subquestions && qData.subquestions.length > 0 ?
                                                                'Parent answer is auto-set from sub-answers' : ''"
                                                            ...
                                                            :class="{
                                                                'bg-green-500 text-white border-green-600 ring-2 ring-green-200': qData
                                                                    .response && qData.response.answer === option &&
                                                                    option === 'Y',
                                                                'bg-amber-500 text-white border-amber-600 ring-2 ring-amber-200': qData
                                                                    .response && qData.response.answer === option &&
                                                                    option === 'P',
                                                                'bg-red-500 text-white border-red-600 ring-2 ring-red-200': qData
                                                                    .response && qData.response.answer === option &&
                                                                    option === 'N',
                                                                'bg-blue-500 text-white border-blue-600 ring-2 ring-blue-200': qData
                                                                    .response && qData.response.answer === option &&
                                                                    option === 'NA',
                                                                'bg-white border-neutral-300 text-neutral-700 hover:bg-neutral-50':
                                                                    !qData.response || qData.response.answer !== option
                                                            }"
                                                            x-text="option">
                                                        </button>
                                                    </template>
                                                ...
                                                <template
                                                    x-if="qData.response && ['P', 'N', 'NA'].includes(qData.response.answer)">
                                                    <div class="space-y-2">
                                                        <div class="relative" x-show="qData.response.answer !== 'NA'">
                                                            <label
                                                                class="block text-xs font-semibold text-neutral-700 mb-1">
                                                                Comment <span class="text-red-500">*</span>
                                                                <span class="ml-1 text-red-500 text-xs"
                                                                    x-show="!qData.tempComment || qData.tempComment.trim() === ''">(Required)</span>
                                                            </label>
                                                            <textarea x-model="qData.tempComment" @input.debounce.500ms="autoSaveResponse(sIndex, qIndex)" rows="3"
                                                                class="block w-full px-3 py-2 border-2 rounded-xl text-sm transition-all focus:ring-2 focus:ring-offset-1"
                                                                :class="{
                                                                    'border-red-300 focus:border-red-500 focus:ring-red-200 bg-red-50':
                                                                        !qData.tempComment || qData.tempComment
                                                                        .trim() === '',
                                                                    'border-neutral-300 focus:border-indigo-500 focus:ring-indigo-200': qData
                                                                        .tempComment && qData.tempComment
                                                                        .trim() !== ''
                                                                }"
                                                        ...
                                                        <template x-if="qData.response.answer === 'NA'">
                                                            <div class="relative">
                                                                <label
                                                                    ...
                                                                    <span class="ml-1 text-red-500 text-xs"
                                                                        x-show="!qData.tempNaJustification || qData.tempNaJustification.trim() === ''">(Required)</span>
                                                                </label>
                                                                <textarea x-model="qData.tempNaJustification" @input.debounce.500ms="autoSaveResponse(sIndex, qIndex)" rows="2"
                                                                    class="block w-full px-3 py-2 border-2 rounded-xl text-sm transition-all focus:ring-2 focus:ring-offset-1"
                                                                    :class="{
                                                                        'border-red-300 focus:border-red-500 focus:ring-red-200 bg-red-50':
                                                                            !qData.tempNaJustification || qData
                                                                            .tempNaJustification.trim() === '',
                                                                        'border-neutral-300 focus:border-indigo-500 focus:ring-indigo-200': qData
                                                                            .tempNaJustification && qData
                                                                            .tempNaJustification.trim() !== ''
                                                                    }"
                                                ...
                                                <div
                                                    class="mt-4 space-y-3 p-4 bg-neutral-50 rounded-xl border border-neutral-200">
                                                    <div class="flex items-center justify-between">
                                                        <h4
                                                            class="text-sm font-semibold text-neutral-700 flex items-center">
                                                            <i class="fas fa-paperclip mr-2 text-indigo-500"></i> Evidence
                                                            Files
                                                        </h4>
                                                        <label
                                                            class="px-3 py-1.5 bg-indigo-600 text-white rounded-lg text-xs font-semibold cursor-pointer hover:bg-indigo-700 transition-colors shadow-sm">
                                                    ...
                                                    <template x-if="qData.evidence && qData.evidence.length > 0">
                                                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                                            <template x-for="(file, fileIdx) in qData.evidence"
                                                                :key="fileIdx">
                                                                <div
                                                                    class="relative group bg-white border-2 border-neutral-200 rounded-xl overflow-hidden hover:shadow-lg transition-all">
                                                                    ...
                                                                    <template x-if="file.type !== 'image'">
                                                                        <div
                                                                            class="w-full h-24 flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-50">
                                                                            <i class="fas fa-file-pdf text-4xl text-indigo-400"
                                                                                :class="{
                                                                                    'fa-file-pdf text-red-400': file
                                                                                        .mime_type === 'application/pdf',
                                                                                    'fa-file-word text-blue-400': file
                                                                                        .mime_type.includes('word'),
                                                                                    'fa-file text-neutral-400': !file
                                                                    ...
                                                                    <div class="p-2 bg-white">
                                                                        <p class="text-xs text-neutral-900 truncate font-medium"
                                                                            x-text="file.display_name"></p>
                                                                        <p class="text-xs text-neutral-500"
                                                                            x-text="formatFileSize(file.file_size)"></p>
                                                                    </div>
                                                                    <div
                                            ...
                                            <div class="space-y-3">
                                                <template x-if="qData.response">
                                                    <div>
                                                        <span
                                                            ...
                                                            :class="{
                                                                'bg-green-100 text-green-800 border-2 border-green-300': qData
                                                                    .response.answer === 'Y',
                                                                'bg-amber-100 text-amber-800 border-2 border-amber-300': qData
                                                                    .response.answer === 'P',
                                                                'bg-red-100 text-red-800 border-2 border-red-300': qData
                                                                    .response.answer === 'N',
                                                                'bg-blue-100 text-blue-800 border-2 border-blue-300': qData
                                                                    .response.answer === 'NA'
                                                            }"
                                                        ...
                                                        <template x-if="qData.response.comment">
                                                            <div
                                                                class="mt-3 p-3 bg-neutral-50 rounded-lg border border-neutral-200">
                                                                <p class="text-xs font-semibold text-neutral-600 mb-1">
                                                                    Comment:</p>
                                                                <p class="text-sm text-neutral-700"
                                                                    x-text="qData.response.comment"></p>
                                                            </div>
                                                        </template>
                                                        <template x-if="qData.response.na_justification">
                                                            <div
                                                                class="mt-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                                                                <p class="text-xs font-semibold text-blue-600 mb-1">NA
                                                                    Justification:</p>
                                                                <p class="text-sm text-blue-700"
                                                                    x-text="qData.response.na_justification"></p>
                                                            </div>
                                                        </template>
                                                    </div>
                                                </template>
                                                ...
                                                <template x-if="qData.evidence && qData.evidence.length > 0">
                                                    <div
                                                        ...
                                                        <h4
                                                            class="text-sm font-semibold text-neutral-700 mb-3 flex items-center">
                                                            <i class="fas fa-paperclip mr-2 text-indigo-500"></i> Evidence
                                                            Files
                                                        </h4>
                                                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                                                                ...
                                                                <a :href="`/storage/${file.file_path}`" target="_blank"
                                                                    ...
                                                                    <template x-if="file.type !== 'image'">
                                                                        <div
                                                                            class="w-full h-24 flex items-center justify-center bg-gradient-to-br from-indigo-50 to-blue-50">
                                                                            <i
                                                                                class="fas fa-file-pdf text-4xl text-indigo-400"></i>
                                                                        </div>
                                                                    </template>
                                                                    <div class="p-2">
                                                                        <p class="text-xs text-neutral-900 truncate font-medium"
...
function auditResponseApp() {
  return {
    ...
    // ---- Modals (defined so they never throw) ----
    confirmDialog: {
      show: false, title: '', message: '', _resolver: null,
      resolve() { if (this._resolver) this._resolver(true); this._cleanup(); },
      reject()  { if (this._resolver) this._resolver(false); this._cleanup(); },
      _cleanup(){ this.show = false; this.title=''; this.message=''; this._resolver=null; }
    },
    evidenceModal: {
      show: false, url: '', type: 'other', fileName: '',
      open(url, fileName, mime) {
        this.url = url; this.fileName = fileName || 'File';
        const m = (mime || '').toLowerCase();
        this.type = m.startsWith('image/') ? 'image' : (m === 'application/pdf' ? 'pdf' : 'other');
        this.show = true;
      },
      close(){ this.show=false; this.url=''; this.fileName=''; this.type='other'; }
    },

    // ---- Lifecycle ----
    init() {
      // Normalize evidence mime_type so template `.includes()` never crashes
      this.sectionsData.forEach(sec => {
        sec.questions.forEach(q => {
          if (Array.isArray(q.evidence)) {
            q.evidence.forEach(f => { f.mime_type = ((f && f.mime_type) ? String(f.mime_type) : '').toLowerCase(); });
          }
        });
      });

      // Pre-color parent rows from existing subs (no writes on load)
      this.sectionsData.forEach(sec => {
        sec.questions.forEach(q => {
          if (this.hasSubs(q) && !(q.response && q.response.answer)) {
            const agg = this._aggregateFromSubs(q);
            if (agg.decision) {
              q.response = { answer: agg.decision };
              q.isAnswered = true;
            }
          } else if (q.response && q.response.answer) {
            q.isAnswered = true;
          }
          this._ensureNAUiNoComment(q); // ensure NA never shows comment required in UI
        });
      });

      this.updateProgress();
      this.updateScrollButtons();
      this._applyNACommentHidingAll(); // hide comment block when NA
    },
    ...
    // ---- Helpers ----
    hasSubs(qData) {
      return Array.isArray(qData.subquestions) && qData.subquestions.length > 0;
    },

    // Keep this name because your template references it
    canSelectYes(qData) {
      if (qData.question && qData.question.requires_all_subs_for_yes !== 1) return true;
      if (!this.hasSubs(qData)) return true;
    ...
    // If answer is NA, ensure UI doesn't show Comment as required (without changing HTML)
    _ensureNAUiNoComment(q) {
      if (q && q.response && q.response.answer === 'NA') {
        if (!q.tempComment || !q.tempComment.trim()) q.tempComment = '—'; // UI placeholder only
      }
    },

    // *** Surgical DOM-based hider: hide the "Comment" block when NA justification is present ***
    _applyNACommentHidingAll() {
      this.$nextTick(() => {
        const root = this.$el || document;
        const labels = root.querySelectorAll('label');
        labels.forEach(lab => {
          const txt = (lab.textContent || '').trim().toLowerCase();
          if (!txt.startsWith('comment')) return;
          const commentBlock = lab.closest('.relative');
          if (!commentBlock) return;
          const wrapper = commentBlock.parentElement; // div.space-y-2
          if (!wrapper) return;
          const naJust = wrapper.querySelector('textarea[x-model$="tempNaJustification"]');
          commentBlock.style.display = naJust ? 'none' : '';
        });
      });
    },

    // Core aggregation from subs (no I/O)
    _aggregateFromSubs(qData) {
      const totalSubs = this.hasSubs(qData) ? qData.subquestions.length : 0;
      if (!totalSubs) return { decision: null, totalAnswered: 0, totalSubs: 0 };

      const counts = { Y:0, P:0, N:0, NA:0, totalAnswered:0 };
      qData.subquestions.forEach(sub => {
        const sr = qData.sub_responses && qData.sub_responses[sub.id];
        const a = sr && sr.answer;
        if (a === 'Y' || a === 'P' || a === 'N' || a === 'NA') { counts[a]++; counts.totalAnswered++; }
      });

      if (counts.totalAnswered === 0) return { decision: null, totalAnswered: 0, totalSubs };

      let decision = 'P';
      if (counts.NA > 0 && counts.Y === 0 && counts.P === 0 && counts.N === 0) decision = 'NA';
      else if (counts.N === 0 && counts.P === 0 && counts.Y > 0) decision = 'Y';
      else if (counts.P > 0 && counts.N === 0) decision = 'P';
      else if (counts.N > 0 && (counts.Y > 0 || counts.P > 0)) decision = 'P';
      else if (counts.N > 0 && counts.Y === 0 && counts.P === 0) decision = 'N';

      return { decision, totalAnswered: counts.totalAnswered, totalSubs };
    },

    _scheduleParentPersist(sIndex, qIndex) {
      const qData = this.sectionsData[sIndex].questions[qIndex];
      if (qData._parentTimer) clearTimeout(qData._parentTimer);
      qData._parentTimer = setTimeout(() => this._computeAndPersistParent(sIndex, qIndex), 350);
    },

    async _computeAndPersistParent(sIndex, qIndex) {
      const qData = this.sectionsData[sIndex].questions[qIndex];
      const { decision, totalAnswered, totalSubs } = this._aggregateFromSubs(qData);
      if (!decision || totalAnswered !== totalSubs) return; // wait until all subs are answered

      if (!qData.response) qData.response = {};
      qData.response.answer = decision;
      qData.isAnswered = true;

      if (decision === 'P' || decision === 'N') {
        if (!qData.tempComment || !qData.tempComment.trim() || qData.tempComment === '—') {
          const pairs = Object.entries(qData.sub_responses || {}).map(([id, sr]) => `${id}:${sr?.answer || '-'}`).join(', ');
          qData.tempComment = `Auto-aggregated from sub-answers: ${pairs}`;
        }
        qData.tempNaJustification = '';
      } else if (decision === 'NA') {
        if (!qData.tempNaJustification || !qData.tempNaJustification.trim()) {
          qData.tempNaJustification = 'All sub-questions are NA';
        }
        qData.tempComment = '—'; // UI placeholder only; never submitted
      } else {
        qData.tempComment = '';
        qData.tempNaJustification = '';
      }

      this._ensureNAUiNoComment(qData);
      this._applyNACommentHidingAll();
      await this._saveParentResponse(sIndex, qIndex);
    },
    ...
    updateProgress() {
      this.sectionsData.forEach(section => {
        ...
        section.answeredCount = section.questions.filter(q => !!(q.response && q.response.answer)).length;
        section.hasUnanswered = section.answeredCount < section.totalCount;
        section.percentage = section.totalCount > 0 ? Math.round((section.answeredCount / section.totalCount) * 100) : 0;
      });
    },
    ...
    async selectAnswer(sIndex, qIndex, answer) {
      ...
      if (this.hasSubs(qData)) {
        this.showToast('Parent answer is auto-set from sub-answers', 'info');
        return;
      }
      ...
      if (answer === 'NA') {
        if (!qData.tempNaJustification) qData.tempNaJustification = '';
        qData.tempComment = '—'; // placeholder for UI only; not submitted
      }
    ...
    async selectSubAnswer(sIndex, qIndex, subId, answer) {
      ...
      if (qData.question && qData.question.requires_all_subs_for_yes === 1 &&
          qData.response && qData.response.answer === 'Y' &&
          (answer === 'P' || answer === 'N')) {
        const ok = await this.showConfirm(
          'Warning: Parent Answer Conflict',
          'This question is currently Y (requires ALL sub-questions to be Y or NA). ' +
          `Setting this sub-question to ${answer} will downgrade the parent to P. Continue?`
        );
        if (!ok) return;
      }

      if (!qData.sub_responses) qData.sub_responses = {};
      if (!qData.sub_responses[subId]) qData.sub_responses[subId] = {};
      qData.sub_responses[subId].answer = answer;

      const ok = await this._saveSubResponse(sIndex, qIndex, subId);
      if (!ok) return;

      this._scheduleParentPersist(sIndex, qIndex);
    },

    // Keep this name because your template calls it in @input.debounce
    async autoSaveResponse(sIndex, qIndex) {
      await this._saveParentResponse(sIndex, qIndex);
    },

    // ---- Persistence (Parent) ----
    async _saveParentResponse(sIndex, qIndex) {
      const qData = this.sectionsData[sIndex].questions[qIndex];
      const answer = qData.response && qData.response.answer ? qData.response.answer : null;
      if (!answer) return;

      // Validation rules:
      if ((answer === 'P' || answer === 'N') && (!qData.tempComment || !qData.tempComment.trim() || qData.tempComment === '—')) {
        return; // need a real comment for P/N
      }
      if (answer === 'NA') {
        // ONLY NA justification is required; do not submit comment even if placeholder exists
        if (!qData.tempNaJustification || !qData.tempNaJustification.trim()) return;
      }
      ...
      // Only send comment for non-NA answers
      if (answer !== 'NA' && qData.tempComment && qData.tempComment.trim() && qData.tempComment !== '—') {
        formData.append('comment', qData.tempComment.trim());
      }
      if (qData.tempNaJustification && qData.tempNaJustification.trim()) {
        formData.append('na_justification', qData.tempNaJustification.trim());
      }

      try {
        const resp = await fetch('{{ route('audits.responses.store') }}', { method: 'POST', body: formData });
        const result = await resp.json();

        if (result && result.success) {
          qData.isAnswered = true;
          qData.response.comment = (answer === 'NA') ? null : (qData.tempComment || null);
          qData.response.na_justification = qData.tempNaJustification || null;

          if (result.progress) this.progress = result.progress;
          if (result.scores) this.scores = result.scores;
          this.updateProgress();
          this._applyNACommentHidingAll(); // keep UI correct after save
          this.showToast('Response saved', 'success');
          return true;
        } else {
          this.showToast((result && (result.error || 'Failed to save')) || 'Failed to save', 'error');
          return false;
        }
      } catch (e) {
        console.error(e);
    ...
    // ---- Persistence (Sub) ----
    async _saveSubResponse(sIndex, qIndex, subId) {
      const qData = this.sectionsData[sIndex].questions[qIndex];
      ...
      try {
        const resp = await fetch('{{ route('audits.subresponses.store') }}', { method: 'POST', body: formData });
        const result = await resp.json();

        if (result && result.success) {
          if (result.parent_invalidated) {
            if (!qData.response) qData.response = {};
            qData.response.answer = result.new_parent_answer || 'P';
            if (result.parent_comment) qData.tempComment = result.parent_comment;
            this.showToast(`Parent answer changed to ${qData.response.answer} due to sub-question conflict`, 'warning');
          } else {
          ...
          // Ensure UI correct (e.g., if parent became NA)
          this._applyNACommentHidingAll();
          return true;
        } else {
          this.showToast((result && (result.error || 'Failed to save')) || 'Failed to save', 'error');
    ...
    showConfirm(title, message) {
      return new Promise((resolve) => {
        ...
        this.confirmDialog._resolver = resolve;
    ...
    async uploadEvidence(event, sIndex, qIndex) {
      ...
      this.isSaving = true;
      try {
        const resp = await fetch('{{ route('audits.evidence.upload') }}', { method: 'POST', body: formData });
        const result = await resp.json();

        if (result && result.success) {
          if (!qData.evidence) qData.evidence = [];
          const ev = result.evidence || {};
          ev.mime_type = ((ev.mime_type) ? String(ev.mime_type) : '').toLowerCase();
          qData.evidence.push(ev);

          this.showToast('Evidence uploaded', 'success');

          const url = `/storage/${ev.file_path}`;
          this.evidenceModal.open(url, ev.display_name, ev.mime_type);
        } else {
          this.showToast((result && (result.error || 'Upload failed')) || 'Upload failed', 'error');
        }
      } catch (e) {
        console.error(e);
        this.showToast('Network error', 'error');
    ...
    showToast(message, type = 'info') {
      ...
      toast.innerHTML = `<i class="fas ${color.icon} mr-3 text-xl"></i><span class="font-semibold">${message}</span>`;
